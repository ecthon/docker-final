...
const Rodape: FC = (): JSX.Element => {
    ...
    useEffect(() => {
        const buscarInfosApi = async () => {
            try {
                const { data } = await ApiService.get<InformacoesApi>(import.meta.env.VITE_ROTA_INFOS_API);
                setInfosApi(data);
            } catch (erro: any) {
                console.log(erro);
            }
            ...
            <RodapeContainer>
                <Box sx={{ maxWidth: "400px" }}>
                    <Typography
                        ...
                        sx={{ mb: 2 }}
                    >
                        Esta aplicação foi construída como exemplo para o módulo de Docker do curso Web Academy da UFAM.
                    </Typography>
                    {infosApi && (
                        <Typography variant="body2" component="h2" sx={{ fontWeight: "bold" }}>
